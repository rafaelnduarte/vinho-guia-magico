...
CREATE OR REPLACE FUNCTION public.get_rankings(period text DEFAULT 'all'::text)
 RETURNS TABLE(user_id uuid, full_name text, avatar_url text, vote_count bigint, comment_count bigint, course_count bigint, total_points bigint, role text, membership_type text)
 LANGUAGE plpgsql
 SECURITY DEFINER
